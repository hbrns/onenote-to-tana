...
def ui_handle_elements(element_name: str, dictionary: Dict[str, ElementTree.Element], handler: Callable) -> bool:
    if dictionary:
        ...
        handler(onenote_app, dictionary)
        return True
    else:
...
if __name__ == "__main__":
    ...
    group.add_argument('-u', '--user', action='store_true', help='Call the function ui_handle_onenote_elements')
    group.add_argument('-a', '--all', action='store_true', help='Call the function handle_onenote_elements')
    ...
    try:
        ...
        if args.user:
            if not narrowed:    # search is not narrowed
                ui_handle_notebooks(onenote_app, notebooks)
            else:               # search is narrowed
                if 'notebook' == narrowed:
                    ui_handle_onenote_elements(onenote_app, notebooks)
                elif 'sections' == narrowed:
                    ui_handle_onenote_elements(onenote_app, notebooks, sections)
                elif 'pages' == narrowed:
                    ui_handle_onenote_elements(onenote_app, notebooks, sections, pages)
                else:
